...
# efficiency of our choice k = 7, K = 7
load("pump.c_chain.RData")
...
# histogram
nclass <- 27
histogram1 <- histogram_c_chains(c_chains_, ndata+1, k, K, nclass = nclass)
niterations <- nrow(chain)
hist_mcmc <- hist(chain[1000:niterations,ndata+1], breaks = histogram1$breaks, plot = F)
g1 <- plot_histogram(histogram1, with_bar = T) + xlab(expression(beta)) + ylab("density")
