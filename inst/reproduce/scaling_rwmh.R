...
#   save(df, file = "dimension.scaling.rwmh.RData")
# }
...
g
ggsave(filename = "dimension.scaling.rwmh1.pdf", plot = g, width = 7, height = 7)
...
#   df <- rbind(df, data.frame(d = d, mean_time = mean(meetingtime), max_time = max(meetingtime),
#                              median_time = median(meetingtime)))
#   save(df, file = "dimension.scaling.rwmh.optimal.RData")
# }
...
g <- ggplot(df, aes(x = d, y = mean_time)) + geom_line() + scale_x_continuous(breaks = unique(df$d)) + ylab("average meeting time")
g
ggsave(filename = "dimension.scaling.rwmh2.pdf", plot = g, width = 7, height = 7)
