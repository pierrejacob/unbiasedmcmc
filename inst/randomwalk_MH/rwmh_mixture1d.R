...
## target distribution
target <- function(x){
  evals <- log(0.5) + dnorm(x, mean = c(-2, 2), sd = 1, log = TRUE)
...
rinit <- function() rnorm(1, mean = 50, sd = 1)
nsamples <- 10000
c_chains_ <-  foreach(irep = 1:nsamples) %dorng% {
  coupled_chains(single_kernel, coupled_kernel, rinit, max_iterations = 1e5)
}
meetingtime <- sapply(c_chains_, function(x) x$meetingtime)
summary(meetingtime)
hist(meetingtime)

length(which(sapply(c_chains_, function(x) is.infinite(x$meetingtime))))
...
histogram <- histogram_c_chains(c_chains_continued_, 1, k, K, breaks = seq(from = -4, to = 4, length.out = 200))
