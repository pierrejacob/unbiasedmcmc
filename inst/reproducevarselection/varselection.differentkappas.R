...
# Unbiased MCMC - meeting times
# nrep <- 100
# df.ue <- data.frame()
# meetingsfilepath <- paste0("varselection.meetings.differentkappas.n", n, ".p", p, ".RData")
#
# for (ikappa in seq_along(kappas)){
#   print(ikappa)
#   kappa <- kappas[ikappa]
#   # load model#   vs <- get_variableselection(Y,X,g,kappa,s0,proportion_singleflip)
#   prior <- vs$prior
#   marginal_likelihood <- vs$marginal_likelihood
#   rinit <- vs$rinit
#   single_kernel <- vs$single_kernel
...
#   df.ue <- rbind(df.ue, data.frame(irep = 1:nrep, ikappa = rep(ikappa, nrep), kappa = rep(kappa, nrep), meetings = meetings))
#   save(df.ue, file = meetingsfilepath)
# }
# load(file = meetingsfilepath)

#
# tail(df.ue)
# df.ue.summary <- df.ue %>% group_by(kappa) %>% summarise(max = max(meetings), q99 = as.numeric(quantile(meetings, prob = 0.99)))
# ks <- 2*floor(df.ue.summary$q99)
# ks <- rep(5e4, nkappas)
# ms <- rep(1e5, nkappas)

# Unbiased MCMC - estimates
# nrep <- 1000
# df.ue.kandm <- data.frame()
# estimatesfilepath <- paste0("varselection.estimates.differentkappas.n", n, ".p", p, ".RData")
#
# for (ikappa in seq_along(kappas)){
#   print(ikappa)
#   kappa <- kappas[ikappa]
...
#   # Unbiased MCMC
#   ues <- foreach(i = 1:nrep) %dorng% {
#     res <- try(unbiasedestimator(single_kernel, coupled_kernel, rinit, h = function(x) x, k = ks[ikappa], m = ms[ikappa]))
...
#   }
#   meetings <- sapply(ues, function(x) x$meetingtime)
#   estimates <- t(sapply(ues, function(x) x$uestimator))
#   df.ue.kandm <- rbind(df.ue.kandm, data.frame(irep = 1:nrep, ikappa = rep(ikappa, nrep), kappa = rep(kappa, nrep), meetings = meetings,
#                                                estimates = estimates))
#   save(df.ue.kandm, file = estimatesfilepath)
# }
# load(file = estimatesfilepath)
# max(df.ue.kandm$irep) # that's nrep
# df.ue.kandm %>% group_by(kappa) %>% summarise(max = max(meetings), q99 = as.numeric(quantile(meetings, prob = 0.99)))
#
#
...
gmcmc
# # gmcmc <- ggplot(df.mcmc[,1:12] %>% gather(component, postmean, postmean.1:postmean.10),
# #        aes(x = kappa, y = postmean, group = component, colour = component)) + geom_line() + scale_color_viridis(discrete = TRUE) +
# #   theme(legend.position = "none")
#
# df.ue.kandm %>% select(ikappa,meetings) %>% group_by(ikappa) %>% summarise(maxmeet = max(meetings))
# df.ue.kandm.postmean <- df.ue.kandm[,1:24] %>% gather(component, postmean, estimates.1:estimates.20)
# df.ue.kandm.postmean <- df.ue.kandm.postmean %>% group_by(ikappa, kappa, component) %>% select(-meetings) %>% summarise(mean = mean(postmean), sd = sd(postmean))
# df.ue.kandm.postmean %>% filter(component == "estimates.1")
#
# df.ue.kandm.postmean %>% head
# df.ue.kandm.postmean$numcomponent <- sapply(df.ue.kandm.postmean$component, getcomponent)
#
# gumcmc <- ggplot(df.ue.kandm.postmean %>% filter(kappa %in% c(0.1,1,2)), aes(x = numcomponent, y = mean, colour = factor(kappa), group = factor(kappa), shape = factor(kappa)))
...
# gumcmc <- ggplot(df.ue.kandm.postmean, aes(x = kappa, y = mean, group = component, colour = component)) + geom_line() + theme(legend.position = "none")
# gumcmc <- gumcmc + scale_color_viridis(discrete = TRUE)
# gumcmc <- gumcmc + geom_errorbar(aes(ymin = mean - 2*sd/sqrt(nrep), ymax = mean + 2*sd/sqrt(nrep)))
#
# gridExtra::grid.arrange(gmcmc, gumcmc, nrow = 1)
#
# df.ue.kandm.postmean$method <- "umcmc"
# dfsub.mcmc$method <- "mcmc"
# dfsub.mcmc %>% head
# as.data.frame(df.ue.kandm.postmean) %>% head
# ggplot(rbind(as.data.frame(df.ue.kandm.postmean), dfsub.mcmc) %>% filter(kappa %in% c(0.1, 2)), aes(x = numcomponent, y = postmean, colour = factor(kappa),
#                                                     group = interaction(method, factor(kappa)), linetype = method)) + geom_point() + geom_line() +
#   scale_color_viridis(name = "kappa: ", discrete = TRUE) + xlab("variable") + ylab("inclusion probability")
